...
async def main():
    ...
    try:
        ...
        # Пробная запись нужна, чтобы сразу поймать режим read-only replica
        probe_key = "fsm:startup_probe"
        await redis.set(probe_key, "ok", ex=30)
        ...
        logging.exception(
            "Redis недоступен для записи, fallback на MemoryStorage: %s", e
        )
