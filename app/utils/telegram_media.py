    ...
    *,
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            LOGGER.warning(
                "telegram.media.%s.network_error chat_id=%s message_id=%s attempt=%s/%s size_bytes=%s duration_ms=%.3f timeout_s=%s proxy_enabled=%s delivery_mode=%s media_key=%s error=%s",
                operation_name,
                chat_id,
                message_id,
                attempt,
        ...
        duration_ms = (perf_counter() - started) * 1000
        LOGGER.info(
            "telegram.media.%s.complete chat_id=%s message_id=%s attempt=%s/%s size_bytes=%s duration_ms=%.3f timeout_s=%s proxy_enabled=%s delivery_mode=%s media_key=%s",
            operation_name,
            chat_id,
            message_id,
            attempt,
...
async def _prepare_photo_reference(photo_bytes: bytes, filename: str):
    if temp_media_enabled():
        ...
        return uploaded_media.url, "url", uploaded_media.key
    return _build_photo(photo_bytes, filename), "multipart", None
...
async def send_photo_with_retry(
    ...
    photo_reference, delivery_mode, media_key = await _prepare_photo_reference(
        photo_bytes,
        filename,
    )
